...
- Hadoop：2年
- Apache Spark：2年
- BI（Tableau・Looker Studio・QuickSight）：3年
- Docker：4年
- Git：6年
- Terraform：1年
- Cloud Formation：1年
...
- データエンジニアリングとデータ分析に対応できます。
- GCP、AWS、Snowflake、dbtなどを活用したデータ基盤の設計・構築・運用経験があります。安くて速いデータパイプライン構築が得意です。
...
- 出版のDX案件（メインの開発者としてBigQueryでデータ基盤を構築）
- 物流のDX案件（メインの開発者としてSnowflake・dbtでデータ基盤を構築）
...
- 100人以上が使用する社内ツールの開発・運用を行い、書籍売上のDX化に貢献（0 -> 1 フェーズ）
- 今後はAI導入やwebアプリケーション開発（Streamlit）などの1 -> 100フェーズを推進する予定
...
- 現在はcsvのデータ連携をIceberg化し、データロードを省略するモダンな構成に取り組んでいます
- 更に今後はdbtのテストを拡充し、データ品質や保守性の向上を推進する予定
